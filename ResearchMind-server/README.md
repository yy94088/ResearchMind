...
服务端会校验 PDF 文件头，使用 Apache PDFBox 提取页数、标题、作者、机构、关键词、
...
中文标题、作者、机构、关键词、摘要、DOI、年份、期刊/会议和研究领域。机构信息
只有能在 PDF 正文节选中找到原文依据时才会采用。AI 可返回一个
...
`/api/admin/**` 只允许 `ADMIN` 角色访问，覆盖全站概览、用户角色与状态、登录/
操作审计和 24 小时前未完成上传清理。鉴权会读取数据库中的实时角色和状态，
因此降权或禁用对已签发令牌立即生效。全新数据库首个注册账户自动成为管理员；
已有数据库可通过 `INITIAL_ADMIN_EMAIL` 指定一次性初始管理员。
